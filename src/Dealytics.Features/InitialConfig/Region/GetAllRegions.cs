...
namespace Dealytics.Features.InitialConfig.Region;

public class GetAllRegions
{
    private readonly IFileDialogService _fileDialogService;

    public GetAllRegions(IFileDialogService fileDialogService)
    {
    ...
    public async Task<List<RegionCategoryDTO>?> Executesync()
    {
        try
        {
            ...
            using (var reader = new StreamReader(stream))
            {
                ...
                var cards = JsonSerializer.Deserialize<List<RegionCategoryDTO>>(jsonContent, new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                });

                return cards ?? new List<RegionCategoryDTO>();
            }
        }
        catch (Exception ex)
        {
